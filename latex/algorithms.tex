...
\caption{Exemplary search results of the algorithms AbYSS, ESPEA, IBEA and MOEA/D on problem instance two for illustrating their performance.}
\label{fig:exruns1}
\end{figure}
...
\caption{Exemplary search results of the algorithms NSGA-II, NSGA-III, SMPSO and SMS-EMOA on problem instance two for illustrating their performance.}
\label{fig:exruns2}
\end{figure}
...
Figure \ref{fig:espea} illustrates the effect of using the charge mechanism in ESPEA. We observe that the density of solutions is higher in those regions that show high efficiencies. At the same time, a focus on the entire front his retained enabling the decision maker to compare is favorite choice with other alternatives.
